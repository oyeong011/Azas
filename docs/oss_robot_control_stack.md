...
8. Real robot motion uses `tools/run_robot_real.sh`, not an ad-hoc launch command, and requires a recent strict live-gate stamp.
...
```bash
STRICT_OPTIONAL=true /home/ssu/Azas/tools/check_oss_stack.sh
```
...
After starting `tools/run_robot_dryrun.sh` or an equivalent live bringup in another terminal, run the field gate checker:
...
`tools/run_robot_real.sh` refuses to launch unless that strict stamp exists, was produced by `STRICT=true`, and is recent.
