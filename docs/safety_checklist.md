...
- `tools/run_robot_dryrun.sh` is allowed for connection checks because it launches with `enable_hardware:=false`.
- Real motion must only use `tools/run_robot_real.sh`; do not use ad-hoc `ros2 launch` commands for real motion.
- `tools/run_robot_real.sh` requires a fresh strict live-gate stamp and the exact typed confirmation `ENABLE_REAL_ROBOT_MOTION`.
...
- Confirm `RUN_LID_STABILITY=true RUN_CUP_STABILITY=true /home/ssu/Azas/tools/field_no_motion_report.sh` reports stable lid and cup/tumbler-body detections.
...
- Confirm `STRICT_LIVE_GATE=true /home/ssu/Azas/tools/field_no_motion_report.sh` passes before `tools/run_robot_real.sh`.
